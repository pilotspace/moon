...
fn xadd_and_xlen() {
    let mut c = sync_conn();
    let _: () = redis::cmd("DEL").arg("compat:stream1").query(&mut c).unwrap();
    let id: String = redis::cmd("XADD")
        .arg("compat:stream1").arg("*").arg("field1").arg("value1")
        .query(&mut c).unwrap();
    assert!(!id.is_empty(), "XADD should return an entry ID");
    let len: i64 = redis::cmd("XLEN").arg("compat:stream1").query(&mut c).unwrap();
    assert_eq!(len, 1);
}
...
fn xrange_basic() {
    let mut c = sync_conn();
    let _: () = redis::cmd("DEL").arg("compat:stream2").query(&mut c).unwrap();
    for i in 0..3 {
        let _: String = redis::cmd("XADD")
            .arg("compat:stream2").arg("*").arg("i").arg(i.to_string())
            .query(&mut c).unwrap();
    }
    let entries: Vec<redis::Value> = redis::cmd("XRANGE")
        .arg("compat:stream2").arg("-").arg("+")
        .query(&mut c).unwrap();
...
fn xtrim_maxlen() {
    let mut c = sync_conn();
    let _: () = redis::cmd("DEL").arg("compat:stream3").query(&mut c).unwrap();
    for i in 0..10 {
        let _: String = redis::cmd("XADD")
            .arg("compat:stream3").arg("*").arg("i").arg(i.to_string())
            .query(&mut c).unwrap();
    }
    let trimmed: i64 = redis::cmd("XTRIM")
        .arg("compat:stream3").arg("MAXLEN").arg("5")
        .query(&mut c).unwrap();
    assert!(trimmed >= 5, "XTRIM should remove at least 5 entries");
    let len: i64 = redis::cmd("XLEN").arg("compat:stream3").query(&mut c).unwrap();
    assert_eq!(len, 5);
}
...
fn eval_keys_and_argv() {
    ...
    let result: String = redis::cmd("EVAL")
        .arg("return redis.call('GET', KEYS[1])").arg(1).arg("compat:lua1")
        .query(&mut c).unwrap();

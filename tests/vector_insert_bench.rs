...
fn bench_raw_append_128d() {
    ...
    println!("Raw append 128d: {n} vectors in {:.2}ms = {vps:.0} vec/s ({us_per:.2} µs/vec)", elapsed.as_millis());
}
...
fn bench_raw_append_768d() {
    ...
    let vps = n as f64 / elapsed.as_secs_f64();
    let us_per = elapsed.as_micros() as f64 / n as f64;
    println!("Raw append 768d: {n} vectors in {:.2}ms = {vps:.0} vec/s ({us_per:.2} µs/vec)", elapsed.as_millis());
}
...
fn bench_full_insert_pipeline_128d() {
    ...
    for i in 0..n {
        ...
        // Append
        let idx = store.get_index_mut(&bytes::Bytes::from_static(b"idx")).unwrap();
    ...
    let vps = n as f64 / elapsed.as_secs_f64();
    let us_per = elapsed.as_micros() as f64 / n as f64;
    println!("Full pipeline 128d: {n} vectors in {:.2}ms = {vps:.0} vec/s ({us_per:.2} µs/vec)", elapsed.as_millis());
}
...
fn bench_full_insert_pipeline_768d() {
    ...
    let vps = n as f64 / elapsed.as_secs_f64();
    let us_per = elapsed.as_micros() as f64 / n as f64;
    println!("Full pipeline 768d: {n} vectors in {:.2}ms = {vps:.0} vec/s ({us_per:.2} µs/vec)", elapsed.as_millis());
}

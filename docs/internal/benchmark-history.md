# moon Benchmark Report — historical record (archive)

**This file is the archive.** It is the full BENCHMARK.md as it stood on
2026-09-07, before the 2026-09-08 re-measurement rewrote the top-level report.
It is kept because every number in it was measured, and a measurement is not
worth less for being superseded — several sections below exist precisely to
document how an earlier published number turned out to be wrong, and deleting
them would delete the correction along with the error.

**Do not quote anything here as a current result.** The current report is
[`BENCHMARK.md`](../../BENCHMARK.md). Numbers here are dated and version-tagged
in place; many are explicitly marked SUPERSEDED or RETRACTED by later sections
*within this file*, and the whole file predates moon `ae6cd003` (v0.8.9).

Read it for: methodology lineage (§2.7.1 strict-vs-loose, §2.11 interleaved
A/B, §2.15.2 benchmark traps), the vector/graph/full-text competitor runs that
have not yet been re-measured, and the audit trail of retracted claims.

...
> **WS5a in-progress note:** db-scoped indexes are a v0.6.0 work-in-progress. Today `FT.CREATE` still tags every new index to db 0 regardless of which db is selected (the connection's selected db isn't threaded into index creation yet), so in practice every index you create is currently a db-0 index and only a `FLUSHDB` issued against db 0 will affect it. FT.SEARCH / FT.INFO / FT._LIST and the HSET auto-index hook are similarly not yet db-scoped — an index is still visible and searchable from every logical db. This is NOT the multi-tenancy isolation guarantee (see [Workspaces](guides/workspaces.md) for the shipped, UUID-prefix-based isolation mechanism) — it is a separate, still-in-progress effort to scope FT indexes to Redis's `SELECT 0..N` logical databases. Full status and the remaining work: `.planning/v0.6.0-release/WS5A-NOTES.md` in the repo.

...
| **Per-key memory footprint** | `CompactKey` 23 B inline SSO; `CompactValue` 16 B SSO with inline TTL; `HeapString` for ≥24 B strings; `listpack` / `intset` / B+tree sorted sets — all custom. **Wins at large values, loses at small ones.** Measured on Linux x86_64 (`BENCHMARK.md` §3.2 — GCE `c3-standard-8`, moon `--shards 1`, vs Redis 7.4.2 built against jemalloc): **15-17 % less RSS per key at ≥ 1 KB values**, a tie at 256 B, and **11-51 % *more* at 32 B**; empty-server RSS is **1.7× Redis** ([#821](https://github.com/pilotspace/moon/issues/821)). The retired "27-35 %" figure came from an Apple M4 Pro rig whose Redis side was measured on macOS and/or without jemalloc — re-measured, moon's own number moved 1.6 % while the oracle moved 12 %. On aarch64 at `--shards 8`, §2.14 measures **1.16× worse at 64 B** and **1.26× worse on idle RSS**. | Embstr threshold raised to **128 B** in 9.1 (PR #1726/#2516) — 20 % less for short strings. Listpack, intset, quicklist, ziplist retained from Redis. | Moon's large-value advantage is confirmed on Linux x86_64 (15-17 % at ≥ 1 KB) and its small-value result is a loss on both arches; Valkey 9.1 raised its embstr threshold to 128 B, which may widen that small-value loss against Valkey specifically — untested. A head-to-head Moon/Valkey memory bench has never been run — every Moon figure here is against Redis. |
...
| **Published throughput** | 5.11M GET/s p=64, 3.50M SET/s p=64 (GCP c3-8, Linux, monoio, **single server**, Apr 2026; Redis `io-threads` setting and payload size not recorded for that run). 2.20× Redis on ARM Neoverse-N1. **GET/SET only** — every other family runs 0.40–0.67× Redis at p≥8 (`BENCHMARK.md` §2.12). | 2.1M RPS (Valkey 9.1, 9 I/O threads, p=10, 512 B, single server). 1B+ RPS on **2000-node cluster** (9.0 target). | **The two numbers are not comparable and no Moon-vs-Valkey ratio is published here**: different hardware, different pipeline depth (64 vs 10), different payload (unrecorded vs 512 B), different thread counts, different harnesses, and Moon's figure is its best two commands. Cluster-scale: Valkey has demonstrated multi-node scale; Moon has not yet published cluster-scale numbers. |
...
- **Per-shard WAL** — AOF throughput advantage grows with pipeline depth (Linux: 1.32× Redis on `everysec` SET p=16, `BENCHMARK.md` §7.3).
...
- **Single-server peak pipelined GET/SET throughput** — no Moon-vs-Valkey benchmark exists; the two vendors' published figures differ in hardware, pipeline depth, payload and thread count, so no ratio is claimed.

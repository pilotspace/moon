...
| **Per-key memory footprint** | `CompactKey` 23 B inline SSO; `CompactValue` 16 B SSO with inline TTL; `HeapString` for ≥24 B strings; `listpack` / `intset` / B+tree sorted sets — all custom. **No current memory win over Redis.** A "27-35 % less RSS at ≥1 KB values" figure was measured on an Apple M4 Pro dev rig and never reproduced on Linux; the Linux measurement (`BENCHMARK.md` §2.14, moon `--shards 8` vs Redis `--io-threads 8`) is 0.94× at 8 B, **1.16× worse at 64 B**, 0.97× at 256 B, **1.26× worse on idle RSS**. | Embstr threshold raised to **128 B** in 9.1 (PR #1726/#2516) — 20 % less for short strings. Listpack, intset, quicklist, ziplist retained from Redis. | Moon's large-value advantage is unproven on Linux and its 64 B result there is a 16% loss; Valkey 9.1 raised its embstr threshold to 128 B. A head-to-head Moon/Valkey memory bench has never been run. |

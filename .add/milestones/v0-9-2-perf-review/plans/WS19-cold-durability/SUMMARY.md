...
- **Scripts over maxmemory (moon#1241):** inside EVAL/EVALSHA, commands that can only free memory (DEL, UNLINK, HDEL, LPOP, EXPIRE, …) are no longer refused with `-OOM`, and a function registered with `allow-oom` runs any command, as in redis 7.0.15. Growing commands in EVAL, and functions without `allow-oom`, are still refused, as in redis.

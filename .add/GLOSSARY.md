...
yield knee (K): the brute-force chunk size (`max_brute_force_vecs_per_chunk`) that trades co-located latency (smaller = finer relief) against search throughput (smaller = more yields); with the cost-free yield it returns to the build-measured 512 — an end-to-end FT.SEARCH A/B (20k×384d, release) put it at +2.74% vs a sync control, 2× margin under the 5% bound and within it for ≥~210d (covers 256d/384d/768d/1536d embeddings) while still yielding ~39×/query; 256 is finer but lands on the 5% line at 384d. Overridable per deployment via `MOON_FT_YIELD_CHUNK`.

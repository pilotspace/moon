...
/// `crate::command::cdc::cdc_read_async`. Stateless — no shard state
/// involved, just reads WAL files from disk (on the CDC read pool, off the
/// shard thread — moon#1181) and decodes them into Debezium JSON
/// envelopes. The push-based CDC.SUBSCRIBE variant (C3b) will live
/// alongside this.
...
pub(super) async fn try_handle_shutdown(
    ...
    if should_save {
        ...
        if let Err(reply) =
            persistence::shutdown_save(&ctx.snapshot_trigger_tx, ctx.num_shards, tokio::time::sleep)
                .await
        {
            responses.push(reply);
            return ShutdownOutcome::Rejected;
...
/// The background merge task is exempt. `FT.COMPACT` / `FT.CONFIG` are exempt
/// from the SEGMENT stall only — see `segment_stall::stall_refusal` (moon#718).
#[inline]

...
use super::{
    ...
    extract_primary_key, handle_blocking_command, handle_config, is_multi_key_command,
    queued_blocking_frame, resp3_shape_for, unpropagate_shard_subscription,
...
>(
    ...
    // Functions API registry — LAZY per SHARD (P-1 footprint): built on first
    // FUNCTION/FCALL/FCALL_RO via `ensure_function_registry`, so a shard that
    // never sees the Functions API pays zero registry + eviction-ctx cost.
    // The eviction ctx (Gap B — FCALL-internal `redis.call` writes run the
    // same OOM gate as EVAL) is built by `ctx.build_lua_eviction_ctx()`.
    //
    // moon#514: per SHARD, not per connection. This used to be a local
    // `Rc::new(RefCell::new(None))`, which scoped the whole Functions API to
    // one TCP connection — `FUNCTION LOAD` on one connection left
    // `FUNCTION LIST` empty on the next.
    ...
    // moon#513 (A2a): batch-scoped fan-out buffers, hoisted OUT of the batch
    // loop and `clear()`ed per batch — the batch loop is a hot path and must
    // not allocate a plan per command. (Its `responses`/`remote_groups`
    // neighbours are still built per batch here; this pair is not, so that the
    ...
    // moon#1177: the per-batch cross-shard buffer and its reply list, hoisted
    // like the fan-out pair above (were a per-batch HashMap keyed by shard —
    // a SipHash, an `Arc` and a ~210 B tuple per remote command — and a
    // per-batch `Vec`). Allocated on first cross-shard use.
    use crate::shard::remote_batch::RemoteMeta;
    let mut remote_groups = crate::shard::remote_batch::RemoteBatch::default();
        ...
        tokio::select! {
            result = async {
                ...
                while frame_idx < num_frames {
                    ...
                    // moon#513 (A2a): `fanout_state` joins the two — a fanned-out
                    // command holds a placeholder in `responses` that only the
                    // end-of-batch fold fills.
                    ...
                    if conn.in_multi && !is_transaction_control(cmd) {
                        ...
                        // Blocking commands must not block at EXEC. Most queue
                        // as their non-blocking twin; the four whose twin
                        // answers a different SHAPE queue unchanged and run in
                        // immediate-only mode at EXEC instead (moon#524).
                        // `queued_blocking_frame` owns that split.
                        ...
                        || cmd.eq_ignore_ascii_case(b"EVALSHA_RO")
                    {
                        ...
                        // moon#515: fan an EVAL'd body out on first sight so
                    ...
                    if !conn.in_multi {
                        ...
                        if is_fcall || cmd.eq_ignore_ascii_case(b"FCALL_RO") {
                            // moon#569: FCALL bodies are ACL-gated per
                            // `redis.call`, same as EVAL. Resolved BEFORE
                            ...
                            crate::shard::slice::with_shard(|s| {
                                ...
                                // blocks to add it; moon#514 needs them
                                // unified because BOTH route through
                                // `route_script_elsewhere` first.
                                if is_fcall {
                    ...
                    // --- CDC.READ ---
                    if dispatch::try_handle_cdc_read(cmd, cmd_args, shaped!()).await {
                        continue;
                    }
                    ...
                    if crate::server::conn::blocking::is_blocking_command_args(cmd, cmd_args) {
                        ...
                        // so it is invalidated below exactly as a delivered
                        // reply; only the write is skipped. The pop is already
                        // logged by the shard that popped (moon#1056).
                    ...
                    // own synchronous stretch before the reply was sent
                    // (`blocking::pop_log`) — see the monoio twin for why it
                    // is never logged here. What stays is the `appendfsync
                    // always` promise: confirm the fsync on the owner's
                    // writer before the reply leaves. A no-op under
                    // `everysec`/`no` and for a reply that popped nothing.
                    if let Some(ref pool) = ctx.aof_pool
                        && let Some(owner) =
                            crate::server::conn::blocking_effect::served_pop_owner(
                                cmd,
                                cmd_args,
                                &blocking_response,
                                ctx.num_shards,
                    ...
                    // slotted.
                    // moon#500 — twin of the monoio guard; see the full
                    // rationale in `handler_monoio/mod.rs`. A multi-key WRITE
                    // inside an active TXN must not reach
                    // `coordinate_multi_key`: that path executes across shards
                    // and bypasses both the undo capture and the cross-shard
                    // TXN guard, so `TXN ABORT` restored nothing. Only a
                    // genuinely SPREAD key set is refused — a single-owner set
                    // falls through to routing, which captures it locally or
                    // refuses it remotely.
                    // moon#500: a multi-key WRITE inside an active TXN must never
                    // enter `coordinate_multi_key`. That path executes the
                    // command itself and returns, bypassing BOTH the undo
                    // capture further down and the cross-shard TXN guard below
                    // it, so `TXN ABORT` restored nothing.
                    //
                    // This branch runs at EVERY shard count here (unlike the
                    // monoio twin, whose enclosing `try_handle_cross_shard_commands`
                    // returns early at `num_shards <= 1`), so at `--shards 1` on
                    // this runtime even a multi-key `DEL` — which the undo
                    // capture handles correctly — never reached the capture and
                    // was silently unrecoverable.
                    //
                    // Standing aside is the fix, not rejecting: ordinary routing
                    // then resolves a single-owner set correctly, reaching the
                    // undo capture when the owner is local and the existing
                    // cross-shard guard when it is not. Only a genuinely SPREAD
                    // set has no correct destination — no cross-shard undo log
                    // exists — so that alone is refused, with the same error and
                    // the same #499 poisoning a cross-shard single-key write gets.
                    let txn_multikey_write = conn.in_cross_txn()
                        && crate::command::metadata::is_write(cmd)
                    ...
                    // moon#513 (A2a): a SPANNING multi-key READ joins the slotted
                    // batch instead of cutting it — twin of the monoio site; see
                    // `handler_monoio/mod.rs` for the full rationale. Gated on
                    // `!in_cross_txn()` outright and placed strictly BELOW the
                    // moon#500 refusal above.
                    //
                    // The monoio/sharded asymmetry is respected without a shard-count
                    // check of its own: at `--shards 1` every key mask has exactly one
                    // bit, so `multikey_placement` answers `Slotted` and this branch
                    // is unreachable — whereas the monoio twin's enclosing
                    // `try_handle_cross_shard_commands` returns early at
                    // `num_shards <= 1`. Both therefore leave one-shard behaviour
                            ...
                            )
                    {
                        ...
                        // Classified on the WHOLE command; applied to the FOLDED
                        // reply, never to a part (moon#460).
                            ...
                            for (target, sub_frame, part_idx) in fanout_scratch.drain(..) {
                                if target == ctx.shard_id {
                                    ...
                                    remote_groups.push(
                                        ...
                                        sub_frame,
                                        RemoteMeta {
                                            sink: crate::server::conn::fanout::ReplySink::Part(part_idx),
                                            // Reads: no AOF barrier, no tracking
                                            // invalidation. A2a touches neither the
                                            // persistence nor the replication path.
                                            persisted_write: false,
                                            track_keys: None,
                                            shape: crate::protocol::resp3::Resp3Shape::None,
                                        },
                                    );
                    ...
                    // moon#592: a write whose argv names a key on a DIFFERENT
                    // shard than the one routing picked is refused BEFORE
                    // routing. Routing sends it to `first_key`'s owner, which
                    ...
                    // moon#1177: only the LOCAL two-db intercepts append these
                    // bytes; a remote leg needs only to know it joins the
                    // owner's barrier (`aof_wanted`) — it used to serialize the
                    // record here too and throw it away.
                    ...
                    if is_local {
                        ...
                        if cmd.eq_ignore_ascii_case(b"MOVE") {
                            ...
                            // moon#1069: the key now exists in `dst_db` — the
                            // wake target, raised once the MOVE is logged
                            // (moon#1056).
                            let mut wake_target: Option<(usize, Bytes)> = None;
                            ...
                            // moon#1056: wake AFTER the MOVE is logged, so a
                            // pop it feeds is logged behind it — whether or
                            // not the AOF leg failed (the key moved anyway).
                        ...
                        if cmd.eq_ignore_ascii_case(b"COPY") {
                            ...
                            if let Some(copy_result) = ksmv::parse_copy_db_args(cmd_args, src_db, db_count) {
                                ...
                                // the wake target, raised once the COPY is
                                // logged (moon#1056).
                                let mut wake_target: Option<(usize, Bytes)> = None;
                        ...
                        if metadata::is_write(cmd) {
                            ...
                            type WriteOutcome =
                                Result<(Frame, crate::blocking::wakeup::ReadyKeys), Frame>;
                            ...
                            let mut do_write = |s: &mut crate::shard::slice::ShardSlice,
                             ...
                             -> WriteOutcome {
                                ...
                                //
                                // moon#1056: only the keys are decided here,
                                // against the registry alone (the same
                                // `ready_keys` gate as the monoio twin). The
                                // wake itself runs once this write is LOGGED,
                                // below: a waiter it serves logs its pop at
                                // the moment it pops, and that record must
                                // follow the write that fed it.
                                let ready = if matches!(response, Frame::Error(_)) {
                            ...
                            // D-2: a keyless flush cleared only this shard; the
                            // broadcast to the others runs below, AFTER this
                            ...
                            // moon#1056: serve the clients blocked on a key
                            // this write touched now that its record is
                            // enqueued, so every pop the wake logs follows it.
                            // Whether or not the AOF leg failed: the data is
                            // in the keyspace either way.
                            crate::blocking::wakeup::wake_ready_keys_on_shard(
                                &ctx.blocking_registry,
                                conn.selected_db,
                                ready,
                            );
                            // D-2: broadcast the flush to every other shard so the
                            // whole keyspace flushes. Any failed leg turns the reply
                            // into an explicit partial-flush error (never silent +OK).
                            //
                            // moon#1084: only now, with this shard's record logged
                            // above. The broadcast awaits, and a write another
                            // client made to THIS shard during that await used to
                            // reach the log BEFORE the flush it followed — replay
                            // then ran the flush over it, and a key acknowledged
                            // after the flush was gone after a restart.
                            if local_flush {
                                ...
                                if ctx.num_shards > 1 {
                                        ...
                                        .await
                                    {
                                        ...
                                        // An AOF failure already replaced the
                                        // reply; it is the more serious of the two.
                                        if !aof_failed {
                                            response = e;
                                        }
                                    }
                        ...
                        {
                            Some(Box::new(crate::tracking::invalidation::written_keys(cmd, cmd_args)))
                        } else {
                            None
                        };
                        ...
                        remote_groups.push(
                            ...
                            dispatch_frame,
                            RemoteMeta {
                                sink: crate::server::conn::fanout::ReplySink::Direct(resp_idx),
                                persisted_write: aof_wanted,
                                track_keys,
                                shape: resp3_shape,
                            },
                        );
                ...
                if !remote_groups.is_empty() {
                    ...
                    reply_futures.clear();
                    // moon#1177: ascending shard order over the shard-indexed
                    // buffer; each target's commands MOVE into its message.
                    for target in 0..remote_groups.slots() {
                        // The db_index captured with the target's first command
                        // (all commands in a pipeline batch targeting the same
                        // shard share it).
                        let Some((commands, meta, batch_db)) = remote_groups.take(target) else {
                            continue;
                        };
                    ...
                    for (mut meta, target) in reply_futures.drain(..) {
                        ...
                        for (RemoteMeta { sink, persisted_write, track_keys, shape: resp3_shape }, resp) in meta.drain(..).zip(shard_responses) {
                            ...
                            // reply, moon#460), no AOF barrier and no tracking
                            ...
                            // write; `persisted_write` is used only for the barrier check.
                ...
                // moon#513 (A2a): fold every fanned-out command's parts into its
                // ONE client reply. Runs AFTER phase 2 has drained (or errored)
                // every part slot and BEFORE the responses are encoded, so no
                // placeholder can reach the wire.
                if !fanout_state.is_empty() {
                    crate::server::conn::fanout::fold(
                        &fanout_state,
                        &mut responses,
                        conn.protocol_version,
                    );

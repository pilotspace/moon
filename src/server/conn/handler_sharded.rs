            ...
            result = async {
                ...
                for frame in batch {
                        ...
                        if ctx.num_shards > 1 {
                                        ...
                                        match crate::command::vector_search::pre_parse_field_filter(query_str.as_ref()) {
                                            Ok(Some(clause)) => {
                                                if let Some(filter) = clause.filter {
                                                    ...
                                                    let response = crate::shard::coordinator::scatter_text_search_filter(
                                                        index_name,
                                                        filter,
                                                        top_k,
                                                        offset,
                                    ...
                                    match crate::command::vector_search::parse_hybrid_modifier(cmd_args) {
                                        ...
                                        Ok(None) => {
                                            if crate::command::vector_search::is_text_query(
                                                ...
                                                // Step 4: index must have at least one TEXT field.
                            ...
                            let response = {
                                ...
                                if cmd.eq_ignore_ascii_case(b"FT.CREATE") {
                                    ...
                                    let mut db_guard = shard_databases_ref.write_db(ctx.shard_id, 0);
                                    crate::command::vector_search::ft_dropindex(&mut vs, &mut ts, Some(&mut *db_guard), cmd_args)
                    ...
                    if cmd.len() > 6 && cmd[..6].eq_ignore_ascii_case(b"GRAPH.") {
                        let (response, wal_records, cypher_intents, cypher_undo_ops) = if crate::command::graph::is_graph_write_cmd(cmd)

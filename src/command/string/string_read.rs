...
pub fn getex(db: &mut Database, args: &[Frame]) -> Frame {
    ...
    if args.len() > 1 {
        ...
        // All arms route through `db.set_expiry` (moon#541): the old raw
        // `get_mut` + `set_expires_at_ms` writes bypassed BOTH the
        // `maybe_has_expiring_keys` latch (a key whose only TTL came from
...
/// LCS key1 key2 [LEN] — read-only twin for the shared-lock path.
///
/// Uses `get_if_alive` so expired keys are invisible and NOT deleted.
pub fn lcs_readonly(db: &crate::storage::db::Database, args: &[Frame], now_ms: u64) -> Frame {
    if args.len() < 2 {
        return err_wrong_args("LCS");
    }
    let key1 = match extract_bytes(&args[0]) {

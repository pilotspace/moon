...
fn zrem_member(members: &mut HashMap<Bytes, f64>, scores: &mut BPTree, member: &[u8]) -> bool {
    ...
    while j < remaining.len() {
        ...
        if should_update {
            ...
            if is_new {
                added += 1;
                changed += 1;
            } else if existing_score.is_some()
                && (existing_score.unwrap() - score).abs() > f64::EPSILON
            {
                changed += 1;
            }
        }

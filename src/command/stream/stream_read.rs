...
/// XLEN key — read-only twin.
pub fn xlen_readonly(db: &crate::storage::db::Database, args: &[Frame], now_ms: u64) -> Frame {
    if args.len() != 1 {
        return err_wrong_args("XLEN");
    }
    let key = match extract_bytes(&args[0]) {
...
/// XRANGE key start end [COUNT count] — read-only twin.
pub fn xrange_readonly(db: &crate::storage::db::Database, args: &[Frame], now_ms: u64) -> Frame {
    if args.len() < 3 {
        return err_wrong_args("XRANGE");
    }
    let key = match extract_bytes(&args[0]) {
...
/// XREVRANGE key end start [COUNT count] — read-only twin.
pub fn xrevrange_readonly(db: &crate::storage::db::Database, args: &[Frame], now_ms: u64) -> Frame {
    if args.len() < 3 {
        return err_wrong_args("XREVRANGE");
    }
    let key = match extract_bytes(&args[0]) {
...
/// XREAD [COUNT count] [BLOCK ms] STREAMS key [key …] id [id …] — read-only twin.
///
/// BLOCK is parsed-and-ignored (same as the mutable twin) — the shared-lock
/// read path is always non-blocking.
pub fn xread_readonly(db: &crate::storage::db::Database, args: &[Frame], now_ms: u64) -> Frame {
    if args.is_empty() {
        return err_wrong_args("XREAD");
    }
    ...
    while idx < args.len() {
        ...
        if arg.eq_ignore_ascii_case(b"COUNT") {
            ...
            idx += 1; // skip the BLOCK token
            if idx >= args.len() {
                return err_wrong_args("XREAD");
            }
            idx += 1; // skip the milliseconds value (ignored)
        } else if arg.eq_ignore_ascii_case(b"STREAMS") {
            idx += 1;
            break;
        } else {
...
pub fn xinfo_readonly(db: &crate::storage::db::Database, args: &[Frame], now_ms: u64) -> Frame {
    ...
    if subcmd.eq_ignore_ascii_case(b"STREAM") {
        let stream = match db.get_stream_if_alive(key, now_ms) {
            Ok(Some(s)) => s,
            Ok(None) => return Frame::Error(Bytes::from_static(b"ERR no such key")),
            Err(e) => return e,
        };
...
/// XPENDING key group [start end count [consumer]] — read-only twin.
pub fn xpending_readonly(db: &crate::storage::db::Database, args: &[Frame], now_ms: u64) -> Frame {
    if args.len() < 2 {
        return err_wrong_args("XPENDING");
    }
    let key = match extract_bytes(&args[0]) {

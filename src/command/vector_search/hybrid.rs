...
//
// The shard computes BM25 (with injected global IDF), dense KNN, and optional
// sparse, and returns THREE separate raw per-stream lists UNFUSED. The
// coordinator unions per stream across shards and calls `rrf_fuse_three`
// exactly once on the unions — preserving single-shard RRF math correctness.
//
// Keys are carried on each result via a `KeyedResult { result, key }` wrapper
// so the coordinator can resolve response keys from shard-preserved Bytes
// without re-doing lookups (W6 — no synthetic `vec:<id>` fallback at the
// coordinator response layer).
...
///
/// Per **D-13** (literal: "Coordinator re-fuses across shards with global RRF
/// on the union"), this function does NOT call `rrf_fuse_three`. It returns
...
///
/// The shard applies weights of `[1.0, 1.0, 1.0]` internally only for
/// determinism across paths — actual weights are applied by the coordinator
/// in its single `rrf_fuse_three` call.
///
/// Errors:
/// - Unknown index → `Frame::Error("ERR unknown index")`
/// - SPARSE on an index lacking that sparse field → `Frame::Error("ERR sparse field not defined in index")` (D-16)
/// - Malformed dense or sparse blob → `Frame::Error(...)`
#[allow(clippy::too_many_arguments)]
pub fn execute_hybrid_search_local_raw_streams(
    ...
    let sparse_results: Vec<SearchResult> = if let Some((sf, sblob)) = sparse {
        match idx.sparse_stores.get(sf.as_ref()) {
            ...
            None => {
                return Frame::Error(Bytes::from_static(
                    b"ERR sparse field not defined in index",
                ));
            }
        }
    } else {
...
/// Per-stream reply codec: THREE separate ranked lists carried as one Frame::Array.
///
/// Frame layout (flat):
///   Array[
...
/// malformed replies to a canonical error (no panics, ever — threat T-152-05-02).
pub fn decode_shard_hybrid_partial(frame: &Frame) -> Option<ShardHybridReply> {
    ...
    Some(ShardHybridReply { bm25, dense, sparse })
}
...
mod tests {
    ...
    // ── Plan 05 Task 1: three-stream codec tests ─────────────────────────────

    /// Build a test `SearchResult` with explicit key_hash and id.
    ...
    fn test_shard_hybrid_reply_roundtrip_preserves_keys() {
        ...
        let mut vec_map: std::collections::HashMap<u64, Bytes> =

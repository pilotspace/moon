...
mod tests {
    ...
    fn test_build_hybrid_response_shape() {
        ...
        let text_index = TextIndex::new(
            ...
            vec![crate::text::types::TextFieldDef::new(Bytes::from_static(b"body"))],
            crate::text::types::BM25Config::default(),
        );

        let frame =
            build_hybrid_local_response(&fused, &key_map, &text_index, 5, 4, 3, 0, 10);
        match frame {
            Frame::Array(items) => {
                // Layout: total, key1, fields1, key2, fields2, "bm25_hits", 5, "dense_hits", 4, "sparse_hits", 3
                assert_eq!(items[0], Frame::Integer(2));
                assert_eq!(items[1], Frame::BulkString(Bytes::from_static(b"doc:alpha")));
    ...
    fn test_build_hybrid_response_vec_fallback() {
        ...
        let frame =
            build_hybrid_local_response(&fused, &key_map, &text_index, 0, 1, 0, 0, 10);
    ...
    fn test_build_hybrid_response_pagination() {
        ...
        let frame =
            build_hybrid_local_response(&fused, &key_map, &text_index, 0, 0, 0, 1, 2);
        if let Frame::Array(items) = frame {

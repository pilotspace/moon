...
///
/// - `since_last_save_secs`: seconds since the last SUCCESSFUL save (or since
///   the server started, if none) — redis's `lastsave`.
/// - `since_last_attempt_secs`: seconds since this task last started a save
///   (or started) — redis's `lastbgsave_try`.
...
///
/// A rule `(secs, changes)` holds when both are reached since the last
/// successful save, as in redis (`serverCron`); after a failed save redis
/// also waits `CONFIG_BGSAVE_RETRY_DELAY` since the last ATTEMPT — so a
/// failed save is retried 5 s later whatever the rule's seconds (moon#1232
/// review: the rule used to be timed from the attempt, so under `--save "60
/// 1"` a failed save was retried after 60 s).
#[must_use]
pub fn save_rule_due(
    rules: &[(u64, u64)],
    since_last_save_secs: u64,
    ...
    let rule_holds = rules
        .iter()
        .any(|&(secs, threshold)| since_last_save_secs >= secs && changes >= threshold);
...
/// due. `last_attempt` moves only when a save actually started; the rule's
/// seconds run from the last successful save (`LASTSAVE`, which any save —
/// BGSAVE, SHUTDOWN's, an auto-save — moves), or from `started_unix`, the
/// task's start, before the first one.
fn auto_save_tick(
    rules: &[(u64, u64)],
    started_unix: u64,
    ...
    // per-thread slots, each write counted by its redis rule —
    // `admin::metrics_setup`, `command::keyspace_changes`), minus their value
    // at the last successful save. It is the number INFO shows, and it moves
    // back only when a save SUCCEEDS (`mark_save_completed`), so nothing here
    // resets it.
    let changes = crate::admin::metrics_setup::rdb_changes_since_last_save();
    let last_save = crate::command::persistence::LAST_SAVE_TIME
        .load(Ordering::Relaxed)
        .max(started_unix);
    let since_save = unix_now_secs().saturating_sub(last_save);
    let since_attempt = last_attempt.elapsed().as_secs();
    let last_ok = BGSAVE_LAST_STATUS.load(Ordering::SeqCst);
    if save_rule_due(rules, since_save, since_attempt, changes, last_ok)
        && !SAVE_IN_PROGRESS.load(Ordering::SeqCst)
        && start_counted_auto_save(snapshot_trigger)
    {
        info!(
            "Auto-save triggered: {} changes in {}s",
            changes, since_save
        );
...
pub async fn run_auto_save_sharded(
    ...
    loop {
        ...
        tokio::select! {
            _ = interval.tick() => {
                auto_save_tick(&rules, started_unix, &mut last_attempt, &snapshot_trigger);
            }
            _ = cancel.cancelled() => {
                info!("Auto-save task shutting down");
                break;
...
mod tests {
    ...
    fn a_rule_fires_on_its_change_count_and_its_time() {
        let rules = [(1, 10)];
        assert!(!save_rule_due(&rules, 1, 1, 9, true), "9 changes < 10");
        assert!(save_rule_due(&rules, 1, 1, 10, true));
        assert!(
            !save_rule_due(&rules, 0, 0, 10, true),
            "the second has not passed"
        );
        assert!(!save_rule_due(&[], 100, 100, 100, true), "no rule, no save");
        // Any rule of several.
        let rules = [(900, 1), (300, 10), (60, 10_000)];
        assert!(!save_rule_due(&rules, 299, 299, 9_999, true));
        assert!(save_rule_due(&rules, 300, 300, 10, true));
        assert!(save_rule_due(&rules, 900, 900, 1, true));
        // The rule's seconds run from the last SUCCESSFUL save, not from
        // the last attempt.
        assert!(save_rule_due(&rules, 900, 0, 1, true));
        assert!(!save_rule_due(&rules, 0, 900, 1, true));
    }
    ...
    fn a_failed_save_is_retried_after_the_redis_delay() {
        let rules = [(1, 1)];
        assert!(!save_rule_due(&rules, 100, 1, 5, false));
        assert!(!save_rule_due(
            &rules,
            100,
            BGSAVE_RETRY_DELAY_SECS,
            5,
            false
        ));
        assert!(save_rule_due(
            &rules,
            100,
            BGSAVE_RETRY_DELAY_SECS + 1,
            5,
            false
        ));
    }

    /// moon#1232 review: under a rule whose seconds exceed the retry delay a
    /// failed save is still retried 5 s after the attempt, as redis 7.0.15
    /// does — the last success is long ago, so the rule itself holds. It
    /// used to wait the rule's 60 s since the attempt.
    #[test]
    fn a_failed_save_is_retried_after_5s_under_a_60s_rule() {
        let rules = [(60, 1)];
        // Last success an hour ago, the failed attempt 6 s ago.
        assert!(save_rule_due(
            &rules,
            3600,
            BGSAVE_RETRY_DELAY_SECS + 1,
        ...
        // A success 30 s ago: the 60 s rule does not hold yet.
        assert!(!save_rule_due(
            &rules,
            30,
            BGSAVE_RETRY_DELAY_SECS + 1,

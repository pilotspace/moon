...
pub enum FileIdSeedError {
    ...
    #[error("cannot list {dir} to find the highest file_id in use: {cause}")]
    ListDir {
        /// The directory.
        dir: PathBuf,
    ...
    #[error("cannot read an entry of {dir} while finding the highest file_id in use: {cause}")]
    ReadEntry {
    ...
    /// The shard manifest exists but could not be opened.
    #[error("cannot open shard manifest {path} to find the highest file_id in use: {cause}")]
    Manifest {
        /// The manifest path.
        path: PathBuf,
        /// The underlying error (in the message, so every log line that
...
pub fn next_file_id_seed(shard_dir: &Path, shard_id: usize) -> Result<u64, FileIdSeedError> {
    let manifest_path = shard_dir.join(format!("shard-{shard_id}.manifest"));
    let manifest_max = match std::fs::metadata(&manifest_path) {
        Ok(_) => ShardManifest::open(&manifest_path)
            .map_err(|cause| FileIdSeedError::Manifest {
        ...
        Err(cause) => {
            return Err(FileIdSeedError::Manifest {
                path: manifest_path,
                cause,
            });
        }
...
mod tests {
    ...
    fn corrupt_manifest_fails_closed() {
        let tmp = tempfile::tempdir().unwrap();
        std::fs::write(tmp.path().join("shard-0.manifest"), b"torn").unwrap();
        assert!(matches!(
            next_file_id_seed(tmp.path(), 0),
            Err(FileIdSeedError::Manifest { .. })
        ));
    }

...
fn main() -> anyhow::Result<()> {
    ...
    // `auto` (the default) declines where the path cannot fire, so the switch
    // never reads as enabled on a leg that would ignore it. The policy is a
    // pure function so it is unit-tested rather than asserted here.

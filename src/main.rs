...
fn main() -> anyhow::Result<()> {
    ...
    maybe_respawn_with_memory_overrides()?;
    ...
    // CLI priority.
    //
    // NOTE: `maybe_respawn_with_memory_overrides()` above only scans *raw*
    // argv — it will NOT see a `memory-arenas-cap`/`memory-thp` value that
    // comes from the conf file. Those settings must be given on the CLI if
    // the jemalloc respawn is needed. This is documented behaviour.
...
/// (see `malloc_conf` above), an optional `narenas` override, and an
/// optional `thp:always` opt-in.
///
/// Pure and side-effect-free — this is the seam `maybe_respawn_with_memory_overrides`
/// uses to compose `--memory-arenas-cap` and `--memory-thp` into a single
/// conf string, and the seam the unit tests below exercise directly (no
/// process re-spawn needed to test the string logic).
///
/// Only reachable (outside `#[cfg(test)]`) on jemalloc builds — non-jemalloc
/// builds never call it, so it's gated the same way to avoid a dead-code
...
/// = `_rjem_`). jemalloc reads `opt.narenas` / `opt.thp` exactly once at init
/// from the symbol **or** the env var (env wins). Calling `mallctl` after
/// init is a documented no-op for these opts. Re-spawning via the platform
/// process-replace call is the only correct path for a CLI override.
///
/// Sentinel `MOON_ARENAS_CAP_APPLIED=1` prevents infinite re-spawn (shared
/// between both flags — a re-spawned child must never re-spawn again).
#[cfg(all(feature = "jemalloc", unix))]
fn maybe_respawn_with_memory_overrides() -> anyhow::Result<()> {
    use std::env;
    ...
    // Lightweight scan of argv for --memory-arenas-cap N (or =N) and
    // --memory-thp. We can't use clap here because clap::parse() requires
    // the full struct, and we need to inject env vars BEFORE jemalloc reads
    // the config. Use args_os() to avoid panicking on non-UTF-8 argv and to
    // preserve the original OsString argv for the re-spawn below
    // (CodeRabbit). Scans the FULL argv (no early break) so both flags are
    // found regardless of order.
    use std::ffi::OsString;
    use std::os::unix::ffi::OsStrExt;
    let args: Vec<OsString> = env::args_os().collect();
    let mut requested_narenas: Option<u32> = None;
    let mut thp_requested = false;
    let mut i = 1;
    while i < args.len() {
        let a = args[i].as_os_str().as_bytes();
        if let Some(rest) = a.strip_prefix(b"--memory-arenas-cap=") {
            requested_narenas = std::str::from_utf8(rest).ok().and_then(|s| s.parse().ok());
            i += 1;
            continue;
        }
        if a == b"--memory-arenas-cap" {
            if i + 1 < args.len() {
                requested_narenas = args[i + 1]
                    .as_os_str()
                    .to_str()
    ...
    let narenas = requested_narenas.unwrap_or(8);
    ...
    let effective_thp = thp_supported(cfg!(target_os = "linux")) && thp_requested;
    if thp_requested && !effective_thp {
        eprintln!(
    ...
    // replace primitive; never returns on success.
...
mod tests {
    ...
    // ── O4: --memory-thp / --memory-arenas-cap conf-string builder ─────────

    #[test]
    fn malloc_conf_default_has_no_thp_opt() {
        let conf = super::build_malloc_conf(8, false);
        assert_eq!(
            conf,
    ...
    fn malloc_conf_thp_only_appends_thp_always() {
        let conf = super::build_malloc_conf(8, true);
    ...
    fn malloc_conf_composes_arenas_cap_and_thp() {
        ...
        let conf = super::build_malloc_conf(4, true);
    ...
    fn malloc_conf_thp_present_iff_flag_set() {
        assert!(!super::build_malloc_conf(16, false).contains("thp:always"));
        assert!(super::build_malloc_conf(16, true).contains("thp:always"));
    }

    #[test]
    fn resolve_override_none_when_matches_baked_in_default() {
        // narenas:8 + no thp is exactly the static default -> no re-spawn.
        assert!(super::resolve_malloc_conf_override(8, false, false).is_none());
    }

    #[test]
    fn resolve_override_some_for_arenas_cap_alone() {
        let conf = super::resolve_malloc_conf_override(4, false, false)
    ...
    fn resolve_override_some_for_thp_alone() {
        ...
        let conf = super::resolve_malloc_conf_override(8, true, false)
    ...
    fn resolve_override_composes_both_flags_in_one_conf() {
        let conf = super::resolve_malloc_conf_override(4, true, false)
    ...
    fn resolve_override_operator_env_wins_over_arenas_cap() {
        ...
        assert!(super::resolve_malloc_conf_override(4, false, true).is_none());
    }

    #[test]
    fn resolve_override_operator_env_wins_over_thp() {
        assert!(super::resolve_malloc_conf_override(8, true, true).is_none());
    }

    #[test]
    fn resolve_override_operator_env_wins_over_both_composed() {
        assert!(super::resolve_malloc_conf_override(4, true, true).is_none());
    }
    ...
    fn thp_supported_only_on_linux() {
        assert!(super::thp_supported(true));
        assert!(!super::thp_supported(false));
    }
    ...
    fn non_linux_thp_request_downgrades_to_no_override() {
        ...
        let is_linux = false;
        let effective_thp = super::thp_supported(is_linux) && thp_requested;
        ...
        assert!(super::resolve_malloc_conf_override(8, effective_thp, false).is_none());
    }
    ...
    fn non_linux_thp_request_still_allows_arenas_cap() {
        ...
        let is_linux = false;
        let effective_thp = super::thp_supported(is_linux) && thp_requested;
        let conf = super::resolve_malloc_conf_override(4, effective_thp, false)
    ...
    fn linux_thp_request_reaches_conf_string() {
        ...
        let effective_thp = super::thp_supported(is_linux) && thp_requested;
        assert!(effective_thp);
        let conf = super::resolve_malloc_conf_override(8, effective_thp, false)

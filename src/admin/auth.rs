...
impl AuthPolicy {
    ...
    pub fn verify_header(&self, header: Option<&str>) -> Result<String, &'static str> {
        ...
        // Parse from the right so user IDs containing `.` (e.g. email
        // addresses, service accounts) still round-trip via `issue()`.
        let (user, b64) = token.rsplit_once('.').ok_or("malformed token")?;
        if user.is_empty() {

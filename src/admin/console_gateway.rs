...
///
/// Returns `Err(gw)` with the rejected gateway if the global slot was already
/// initialized — callers should treat this as a bootstrap bug, not a recovery
/// path. The already-installed gateway stays in place.

...
fn pre_populate(addr: &str, total_keys: usize, data_size: usize) {
    ...
    let (batch, mut cmd_buf, mut read_buf) =
        (500, Vec::with_capacity(500 * 64), vec![0u8; 64 * 1024]);
    let mut sent = 0;
...
fn run_client(
    ...
    while warmed < warmup {
        ...
        for _ in 0..n {
            let key = if cmd == "get" {
                format!("key:pre:{}", seq % total as u64)
            } else {
                format!("key:{tid}:{seq}")
            };
    ...
    loop {
        ...
        for i in 0..batch {
            let key = if cmd == "get" {
                format!("key:pre:{}", (claimed + i) % total)
            } else {
                format!("key:{tid}:{seq}")
            };
            build_command(cmd, &key, &value, &mut cmd_buf);
...
fn main() {
    ...
    if cmd == "get" {
        if !args.csv {
            eprintln!("Pre-populating {} keys...", args.requests);
        }
    ...
    let handles: Vec<_> = (0..args.clients)
        .map(|tid| {
            ...
            let (pl, ds, total, wu) = (
                ...
                args.data_size,
                args.requests,
                args.warmup / args.clients,
            );
            std::thread::spawn(move || {
                run_client(&addr, &cmd, pl, ds, &counter, total, tid, &barrier, wu)
            })
        })
        .collect();

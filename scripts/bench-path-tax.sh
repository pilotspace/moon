...
run() { # run <inline|generic> <depth>
  local leg="$1" depth="$2" n before after rps delta
  case "$depth" in 8) n=400000 ;; 64) n=1500000 ;; esac
  before="$(inline_count)"
  if [[ "$leg" == inline ]]; then
    rps="$(redis-benchmark -p "$PORT" -n "$n" -c 50 -P "$depth" -r "$KEYSPACE" -q \
           get key:__rand_int__ 2>/dev/null | tr '\r' '\n' \
    ...
    # silently yields an empty leg.
    rps="$(redis-benchmark -p "$PORT" -a benchpw --user bench -n "$n" -c 50 -P "$depth" -r "$KEYSPACE" -q \
           get key:__rand_int__ 2>/dev/null | tr '\r' '\n' \
           | awk '{for(i=1;i<=NF;i++) if($i=="requests"&&$(i+1)=="per"){v=$(i-1);gsub(/,/,"",v);print v;exit}}')"
  fi
  after="$(inline_count)"
  ...
  if [[ "$leg" == inline ]] && (( delta <= 0 )); then
    echo "FATAL: inline leg did not advance local_inline -- it was not inline." >&2; exit 1
  fi
  if [[ "$leg" == generic ]] && (( delta != 0 )); then
    echo "FATAL: generic leg advanced local_inline by $delta -- user still unrestricted." >&2; exit 1
  fi
  echo "$leg,$depth,$rps,$before,$after,$delta"
}

for rep in $(seq 1 "$REPS"); do
  if (( rep % 2 == 1 )); then legs=(inline generic); else legs=(generic inline); fi

...
- Startup index reconcile — why a restart is slow, the per-word cost model, and five disproved explanations: [`docs/internal/startup-reconcile-cost-model.md`](docs/internal/startup-reconcile-cost-model.md). **Read before optimising startup.** 44.4x bench-vs-production, fully decomposed: 5.80x is macOS-vs-Linux (same commit, same data), 1.66x the corpus, the rest binary+cache. Index count and docs-per-index cost nothing.

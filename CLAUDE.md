...
- Startup index reconcile — why a restart is slow, the per-word cost model, and five disproved explanations: [`docs/internal/startup-reconcile-cost-model.md`](docs/internal/startup-reconcile-cost-model.md). **Read before optimising startup: the text plane and its duplicate pass are 96.5% of it, and the vector engine is 3.5%.**

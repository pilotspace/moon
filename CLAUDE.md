...
- Startup index reconcile — why a restart is slow, the per-word cost model, and five disproved explanations: [`docs/internal/startup-reconcile-cost-model.md`](docs/internal/startup-reconcile-cost-model.md). **Read before optimising startup.** Text analysis is 96.5% of the bench reconcile, but the "vector engine is only 3.5%" split in it is a trap the doc explains, and the production instance is 47x slower than the bench for reasons nobody has measured yet.

 ...
 *
 * These are wall-clock performance tests that depend on WebGL + worker
 * throughput. They are inherently flaky on shared CI runners (no GPU,
 * variable CPU) and on headless Chromium (no WebGL canvas). Skip in CI;
 * run locally or in a dedicated perf-test job with GPU runners.
 */

const isCI = !!process.env.CI;
test.setTimeout(60_000);

test.skip(isCI, "perf budgets require GPU + dedicated runner");

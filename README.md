...
| **v0.8.10** (current) | The durability campaign — 144 PRs closing the 2026-09 issue wave: the cold tier, AOF rewrite fold and WAL v3 replay each apply a write exactly once across kill -9; blocking pops and consumer-group reads/claims are logged as the change they made, on the shard that owns the key; vector and text documents survive restart, rescan and re-write; a crash-orphaned cold file id is never re-issued; RUSTSEC-2026-0293 (`ringbuf`) cleared | **GA** |

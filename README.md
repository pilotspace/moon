...
| **v0.8.9** | The propagation gate — a command must not write what it cannot propagate. A Lua `nil`/boolean/table argument is refused at the boundary instead of being half-applied by nine commands; `XADD` stops creating a stream it then rejects; and blocking pops (`BLPOP`/`BRPOP`/`BLMOVE`/`BZPOPMIN`/…) reach the AOF and replicas instead of being undone by the next restart | **GA** |
| **v0.8.10** (current) | The durability campaign — 147 PRs closing the 2026-09 issue wave: the cold tier, AOF rewrite fold and WAL v3 replay each apply a write exactly once across kill -9; blocking pops and consumer-group reads/claims are logged as the change they made, on the shard that owns the key; vector and text documents survive restart, rescan and re-write; a crash-orphaned cold file id is never re-issued; RUSTSEC-2026-0293 (`ringbuf`) cleared; the #1055 cold-index eviction gate reverted after the release soak caught it refusing every write under `allkeys-lru`; the release soak also found and fixed an AOF rewrite that could stall for good under sustained writes (#1158) | **GA** |

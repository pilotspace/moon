...
// moon patch: CQ spin budget from MOON_URING_SPIN_US (0 = disabled), read once.
// See inner_park below for the poll-mode rationale.
fn moon_spin_budget_us() -> u64 {
    static SPIN_US: std::sync::OnceLock<u64> = std::sync::OnceLock::new();
    *SPIN_US.get_or_init(|| {
        std::env::var("MOON_URING_SPIN_US")
            .ok()
...
impl IoUringDriver {
    ...
    fn inner_park(&self, timeout: Option<Duration>) -> io::Result<()> {
        ...
        if need_wait {
            // ---- moon patch: CQ spin-poll before blocking (MOON_URING_SPIN_US) ----
            // Poll-mode reaping for request/response workloads: submit pending
            // SQEs immediately (one enter), then busy-poll the completion queue
            // (shared-memory head/tail check, zero syscalls) for a bounded
            // window before falling back to the stock blocking wait. When a
            // completion lands inside the window the thread never sleeps, so
            // the scheduler wakeup disappears from the per-op path. Foreign
            // (cross-thread) wakes during the window are delayed at most by the
            // window: the eventfd is already written, so the stock fallback's
            // submit_and_wait returns immediately. Off unless MOON_URING_SPIN_US
            // is set; 0 behavior change otherwise.
            let spin_us = moon_spin_budget_us();
            if spin_us > 0 {
